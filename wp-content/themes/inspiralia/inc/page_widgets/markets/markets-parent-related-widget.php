...
function markets_parent_related_meta_box_markup($post) { ?>
 <?php wp_nonce_field( basename( __FILE__ ), 'markets_parent_related_meta_box_nonce' );
  $args = array(
      'depth'                 => 2,
...
function add_markets_parent_related_meta_box() {
  global $post;
  if (  'templates/markets.php' == get_post_meta( $post->ID, '_wp_page_template', true ) && ( count(get_ancestors( $post->ID, 'page', 'post_type' ) ) == 0 ) )  {
    add_meta_box("markets_parent_related_meta_box", "Hero Related Element Box", "markets_parent_related_meta_box_markup", "page", "normal", "high", null);
  }
}

  add_action("add_meta_boxes", "add_markets_parent_related_meta_box");


function save_markets_parent_related_meta_box($post_id, $post, $update)
{
...
add_action("save_post", "save_markets_parent_related_meta_box", 10, 3);
