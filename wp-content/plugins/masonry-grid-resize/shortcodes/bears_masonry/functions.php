...
{	
	...
	array_push( $scripts, array(
		...
		'src' => 'http://osvaldas.info/examples/image-lightbox-responsive-touch-friendly/imagelightbox.min.js',
		'deps' => array( 'jquery' ),
		'include' => 1,
		) );
...
function tbbs_BearsMasonryParams()
{
	return array(
		array(
			...
			'type' => 'text',
			'value' => 'category',
			'description' => '<i>Note: Use for custom post type. (post default: category, woocommerce: product_cat, ...)</i>',
			),
		array(
			'name' => 'layout_item',
...
function tbbs_MasonyLayoutCreativeRender( $atts )
{
	...
	/* get filter HTML */
	$filter_Html = tbbs_MasonryFilterHeaderRender( $atts );
	$container_Html = tbbs_MasonryContentRender( $atts );
 ...
 * @param array $atts
 */
function tbbs_MasonryFilterHeaderRender( $atts )
{
	/* wp_query */
    $_post = tbbs_MasonryQueryPost( $atts['source'] );

	if( (int) $atts['template_params']['display_filter'] == false ) return $output;

	$filterDatas = tbbs_MasonryGetFilterDataByTaxonomy( $_post, ( ! empty( $atts['template_params']['filter_by_taxonomy'] ) ? $atts['template_params']['filter_by_taxonomy'] : 'category' ) );
 ...
 *
 */
function tbbs_MasonryContentRender( $atts )
{
	extract( $atts['settings'] );

	$items_Html = tbbs_MasonryLoopItems( $atts );
 ...
 *
 */
function tbbs_MasonryLoopItems( $atts, $paged = 1 ) 
{
	extract( $atts['settings'] );

	/* wp_query */
    $posts = tbbs_MasonryQueryPost( $atts['source'], $paged );
...
{	
	extract( $_POST );
	echo tbbs_MasonryLoopItems( $atts, $paged );
	wp_die();
}
